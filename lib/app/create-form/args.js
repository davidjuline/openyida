...
const CREATE_FORM_LAYOUTS = new Set(['single', '1', 'double', '2', 'card', 'section']);
const CREATE_FORM_THEMES = new Set(['default', 'compact', 'comfortable']);
const CREATE_FORM_LABEL_ALIGNS = new Set(['top', 'left', 'right']);
  ...
  return function parseArgs(inputArgs) {
    ...
    for (let i = 0; i < args.length; i++) {
      if (args[i] === '--layout') {
        ...
        if (!value || value.startsWith('--') || !CREATE_FORM_LAYOUTS.has(value)) {
          throwCreateFormError('--layout must be one of: single, double, card, section', 'CREATE_FORM_INVALID_ARGUMENTS');
        }
        ...
        const value = args[i + 1];
        if (!value || value.startsWith('--') || !CREATE_FORM_THEMES.has(value)) {
          throwCreateFormError('--theme must be one of: default, compact, comfortable', 'CREATE_FORM_INVALID_ARGUMENTS');
        }
        options.theme = value;
        args.splice(i, 2);
        i--;
      } else if (args[i] === '--label-align') {
        const value = args[i + 1];
        if (!value || value.startsWith('--') || !CREATE_FORM_LABEL_ALIGNS.has(value)) {
          throwCreateFormError('--label-align must be one of: top, left, right', 'CREATE_FORM_INVALID_ARGUMENTS');
        }
        options.labelAlign = value;
        args.splice(i, 2);
        i--;
      } else if (args[i] === '--icon') {
        const value = args[i + 1];
        ...
        args.splice(i, 2);
        i--;
      } else if (args[i] === '--locale' || args[i] === '--content-locale' || args[i] === '--lang') {
        const value = args[i + 1];
        if (!value || value.startsWith('--')) {
          throwCreateFormError(`${args[i]} requires a value`, 'CREATE_FORM_INVALID_ARGUMENTS');
        }
        options.contentLocale = value;
