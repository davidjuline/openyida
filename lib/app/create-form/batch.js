...
const BATCH_PLAN_EXAMPLE = '{"forms":[{"key":"customer","title":"客户","fieldsFile":"customer-fields.json","layout":"section","theme":"comfortable","labelAlign":"left"},{"key":"order","title":"订单","fieldsFile":"order-fields.json","dependsOn":["customer"]}]}';
...
function validateStaticDefinition(form, fields) {
  ...
  for (const [key, values] of [
    ['layout', CREATE_FORM_LAYOUTS],
    ['theme', CREATE_FORM_THEMES],
    ['labelAlign', CREATE_FORM_LABEL_ALIGNS],
  ]) {
    if (form[key] !== undefined && !values.has(form[key])) {
      invalid({
        formKey: form.key,
        errorCode: 'CREATE_FORM_INVALID_ARGUMENTS',
        errorMsg: `Unsupported ${key}: ${form[key]}`,
        [key]: form[key],
      });
    }
  }
...
function loadPlan(file, options = {}) {
  ...
  const forms = input.forms.map(form => {
    ...
    if (Object.keys(form).some(key => !['key', 'title', 'fields', 'fieldsFile', 'dependsOn', 'formUuid', 'layout', 'theme', 'labelAlign', 'icon', 'locale'].includes(key))) { invalid(form.key); }
    if (form.formUuid !== undefined && (typeof form.formUuid !== 'string' || !form.formUuid.startsWith('FORM'))) { invalid(form.key); }
    for (const key of ['layout', 'theme', 'labelAlign', 'icon', 'locale']) { if (form[key] !== undefined && typeof form[key] !== 'string') { invalid(key); } }
...
function expectedReadbackFields(fields) {
  ...
  for (const field of Array.isArray(fields) ? fields : []) {
    ...
    if (['Divider', 'RichText'].includes(field.type)) { continue; }
    ...
    await schedule(forms, options.concurrency, state.results, async form => {
      ...
      if (!formUuid) {
        ...
        for (const [key, option] of Object.entries({
          ...
          if (form[key]) { argv.push(option, form[key]); }
        }
        try {
