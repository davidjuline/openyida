...
const FORM_PRESENTATION_COMPONENTS = ['Divider', 'ColumnsLayout', 'Column'];
...
function normalizeFormInjectionKey(key) {
  return String(key || '').replace(/[^a-z0-9]/gi, '').toLowerCase();
}

function isRawFormInjectionKey(key) {
  const normalized = normalizeFormInjectionKey(key);
  return normalized === 'srcdoc'
    || normalized === 'stylesheet'
    || normalized === 'stylesheets'
    || normalized === 'styletext'
    || normalized === 'styletag'
    || normalized.includes('innerhtml')
    || normalized.includes('outerhtml')
    || normalized.includes('dangerouslysetinnerhtml')
...
function isRawMarkupComponent(componentName) {
  return ['html', 'iframe'].includes(String(componentName || '').toLowerCase());
}

function assertNoFormInjectionValue(value, location) {
  if (Array.isArray(value)) {
    value.forEach(function (item, index) {
      assertNoFormInjectionValue(item, location + '[' + index + ']');
    });
    return;
  }
  if (!value || typeof value !== 'object') {
  ...
  Object.keys(value).forEach(function (key) {
    if (isRawFormInjectionKey(key)) {
      throw new Error('表单补丁不支持 CSS/HTML 注入: ' + location + '.' + key);
    }
    if (normalizeFormInjectionKey(key) === 'componentname' && isRawMarkupComponent(value[key])) {
      throw new Error('表单补丁不支持 CSS/HTML 注入组件: ' + value[key]);
    }
    assertNoFormInjectionValue(value[key], location + '.' + key);
  });
}

function assertNoFormInjectionPath(pointer, value) {
  if (!pointer) {
    return;
  }
  const segments = splitJsonPointer(pointer);
  const blockedSegment = segments.find(isRawFormInjectionKey);
  if (blockedSegment) {
    throw new Error('表单补丁不支持 CSS/HTML 注入路径: ' + pointer);
  }
  const lastSegment = normalizeFormInjectionKey(segments[segments.length - 1]);
  if (lastSegment === 'componentname' && isRawMarkupComponent(value)) {
    throw new Error('表单补丁不支持 CSS/HTML 注入组件: ' + value);
  }
}

function assertNoFormInjectionSource(source, location) {
  if (typeof source !== 'string' || source.length === 0) {
    return;
  }
  const forbiddenPatterns = [
    /\b(?:innerHTML|outerHTML|insertAdjacentHTML|dangerouslySetInnerHTML|srcdoc|cssText|insertRule|adoptedStyleSheets)\b/i,
    /\bdocument\s*\.\s*(?:write|writeln)\s*\(/i,
    /\bcreateElement\s*\(\s*['"`](?:style|script|iframe|link)['"`]\s*\)/i,
    /\bsetAttribute\s*\(\s*['"`](?:style|srcdoc)['"`]/i,
    /\.\s*style\s*(?:\.|\[)/i,
    /<\s*(?:style|script|iframe|html|body)\b/i,
    /openyida:theme|yida-global-theme/i,
  ];
  if (forbiddenPatterns.some(function (pattern) { return pattern.test(source); })) {
    throw new Error('表单动作不支持 CSS/HTML 注入: ' + location);
  }
}
...
function applyAtomicFieldAction(schema, formContainer, operation) {
  ...
  if (source !== undefined) {
    assertNoFormInjectionSource(source, 'field-action ' + actionName);
    ...
    const nextActionSource = upsertGeneratedSourceBlockWithBounds(
      actions.module.source || '',
      source,
      blockStart,
      blockEnd
    );
    assertNoFormInjectionSource(nextActionSource, 'field-action module');
    actions.module.source = nextActionSource;
...
function applySchemaPatchOperations(schema, operations) {
  ...
  operations.forEach(function (operation, index) {
    ...
    assertNoFormInjectionPath(operation.path, operation.value);
    if (Object.prototype.hasOwnProperty.call(operation, 'value')) {
      assertNoFormInjectionValue(operation.value, '补丁[' + index + '].value');
    }
    if (operation.props !== undefined) {
      assertNoFormInjectionValue(operation.props, '补丁[' + index + '].props');
    }
    ...
    if (action === 'actions-module') {
      ...
      assertNoFormInjectionSource(source, 'actions-module source');
      assertNoFormInjectionSource(compiled, 'actions-module compiled');
      if (source !== undefined) {
