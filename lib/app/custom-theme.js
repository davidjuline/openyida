...
const { REQUIRED_BRAND_SCALE_TOKENS, REQUIRED_APPLICATION_BRAND_TOKENS, rootDeclarations, invalidBrandValues } = require('../core/theme-brand-scale');
...
function extractThemeColor(cssText) {
  ...
  const value = rootDeclarations(source)['--color-brand1-6'];
  if (!value) {
...
function validateThemeCssContent(cssText) {
  ...
  // Custom properties accept arbitrary text. An authoring formula can therefore
  // override a valid platform color yet invalidate every property consuming it;
  // var() fallbacks do not rescue that case. Reject it before any upload.
  for (const declaration of sourceWithoutComments.matchAll(/(?:^|[;{])\s*(--[\w-]+)\s*:\s*([^;]+)/g)) {
    if (/\{\{[^}]+\}\}|<生成实际色值[：:]/.test(declaration[2])) {
      throw new CliError(t('design_document.theme_css_unresolved', declaration[1]), {
        code: 'THEME_CSS_UNRESOLVED_TOKEN',
        details: { token: declaration[1], value: declaration[2].trim() },
      });
    }
  }
  ...
  const tokens = rootDeclarations(sourceWithoutComments);
  const missingTokens = REQUIRED_APPLICATION_BRAND_TOKENS.filter((token) => !(token in tokens));
  if (missingTokens.length > 0) {
    throw new CliError(
      `自定义主题文件必须在顶层 :root 完整声明平台品牌色阶 ${REQUIRED_APPLICATION_BRAND_TOKENS.join('、')}，缺少: ${missingTokens.join(', ')}`,
      { code: 'THEME_BRAND_SCALE_INCOMPLETE', details: { missingTokens, scope: ':root' } }
    );
  }
  const issues = invalidBrandValues(tokens, normalizeThemeColor);
  if (issues.length) {
    throw new CliError(t('design_document.invalid', issues[0].token, issues[0].reason), {
      code: 'THEME_BRAND_SCALE_INVALID', details: { issues, scope: ':root' },
    });
  }
}
