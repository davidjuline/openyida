...
function compileCanvasLocal(source, options = {}) {
  ...
  const warnings = [];
  assertCanvasThemeStructure(source, { ...options, onThemeWarning(message, details) {
    warnings.push({ ...details, message });
    if (options.onThemeWarning) { options.onThemeWarning(message, details); }
    else { require('../core/chalk').warn(message); }
  } });
