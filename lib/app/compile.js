...
async function run(args) {
  ...
  const strictBrand = args.includes('--strict-theme');
  ...
  if (filteredArgs.length !== 1 || filteredArgs[0].startsWith('--') || (strictBrand && allowFixedBrand)) {
    throwUsage(t('cli.compile_usage'), t('cli.compile_example'), {
      code: 'COMPILE_INVALID_ARGUMENTS',
    });
  }
  ...
  if (canvas || /\.canvas\.(?:jsx?|tsx?)$/i.test(sourcePath)) {
    ...
    let compiled;
    try { compiled = compileCanvasPageSource(loadedSource, { strictBrand, allowFixedBrand }); }
    catch (error) { throw require('../core/source-repair').withSourceRepair(error, [sourcePath], args); }
    const payload = {
      ok: true,
      pageType: 'canvas',
  ...
  if (strictBrand || allowFixedBrand) {
    throwUsage(t('publish.theme_canvas_only'), undefined, { code: 'COMPILE_INVALID_ARGUMENTS' });
  }
