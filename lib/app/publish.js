...
// Ask the human (via stdin) whether to migrate. Returns false unless both the
// input and the prompt's output stream are real TTYs, so agent/CI/piped runs
// never block on a question nobody can answer and must opt in with --fix-theme.
...
function confirmFixedBrandMigration() {
  if (!canPromptInteractively()) { return Promise.resolve(false); }
...
// or the user declined. Callers continue publishing regardless unless strict.
async function offerFixedBrandThemeMigration(sourcePath, source, { autoConfirm, strict } = {}) {
  ...
  if (!proceed) {
    // Only nudge harder in strict mode; the relaxed path prints its own note.
    if (strict) {
      if (!canPromptInteractively()) { info(t('publish.fix_theme_flag_hint')); }
...
async function runMain(argv) {
  ...
  if (isCanvas) {
    ...
    // Offer to un-pin fixed brand colors before compiling so the ask-human /
    // --fix-theme migration still runs now that the guard no longer blocks by
    // default. Skipped when --allow-fixed-brand explicitly ships as-is. In
    // non-TTY runs the prompt declines instead of hanging, and the guard warns
    // (or hard-fails under --strict-theme) rather than blocking the user.
    if (!allowFixedBrand) {
      const migratedSource = await offerFixedBrandThemeMigration(sourcePath, initialSourceCode, {
        autoConfirm: fixTheme,
        strict: strictTheme,
      });
      if (migratedSource !== null) { sourceCode = migratedSource; }
    }
    try {
