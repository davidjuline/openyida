      ...
      if (error.statusCode === 400 || error.statusCode === 429) {
        ...
        if (options.onState) {
          options.onState({
            ...
            body: typeof payload.error_description === 'string'
              ? payload.error_description
              : (payload.errorMsg || payload.raw || '').slice?.(0, 200),
          });
        }
        await waitFor(deadline, currentInterval, options);
        continue;
...
async function runDeviceCodeFlow(options = {}) {
  ...
  const verificationUri = resolveUrl(code.verification_uri);
  const verificationUriComplete = resolveUrl(code.verification_uri_complete);
